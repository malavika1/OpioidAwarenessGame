...
public class GameState : AState
{
    ...
    public override void Enter(AState from)
    {
        ...
        if (MusicPlayerRunner.instance.GetStem(0) != gameTheme)
        {
            MusicPlayerRunner.instance.SetStem(0, gameTheme);
            CoroutineHandler.StartStaticCoroutine(MusicPlayerRunner.instance.RestartAllStems());
        }
    ...
    protected void UpdateUI()
    {
		...
		if(trackManager.worldDistance > 750 && ! ended)
        {
            MusicPlayerRunner mp = FindObjectOfType<MusicPlayerRunner>();
            Destroy(mp.gameObject);
			ended = true;
            Initiate.Fade("Scene9", Color.white, 0.5f);
        }
    ...
    {
        if (Int32.Parse(distanceText.text.TrimEnd('m')) < 1000)
        {
            manager.SwitchState("GameOver");
        }
    }
